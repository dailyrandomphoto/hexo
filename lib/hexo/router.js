...
Router.prototype.set = function(path, data, noEvent) {
  if (typeof path !== 'string') throw new TypeError('path must be a string!');
  ...
  if (!noEvent) this.emit('update', path);

  return this;
};

Router.prototype.remove = function(path, noEvent) {
  if (typeof path !== 'string') throw new TypeError('path must be a string!');
  path = this.format(path);

  this.routes[path] = null;
  if (!noEvent) this.emit('remove', path);
